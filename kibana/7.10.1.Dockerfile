...
RUN architecture=$(case ${TARGETPLATFORM} in "linux/amd64") echo "x86_64" ;; linux/arm64) echo "aarch64" ;; *) echo "x86_64" ;; esac) && \
    curl -sL https://artifacts.elastic.co/downloads/kibana/kibana-oss-7.10.1-linux-${architecture}.tar.gz | tar --strip-components=1 -zxf -
RUN rm -rf /usr/share/kibana/node/* && \
    ...
    curl -sL https://nodejs.org/dist/v10.22.1/node-v10.22.1-linux-${architecture}.tar.gz | tar -C /usr/share/kibana/node/ --strip-components=1 -xzf -
